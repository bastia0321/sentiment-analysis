...
def build_textcat_model(tok2vec, nr_class):
...
def save_model(nlp, pretrain_epoch):
    path = './pretrained-model/' + str(pretrain_epoch)
...
def train_textcat(nlp, num_eval, opt_params,
    ...
    with nlp.disable_pipes(*other_pipes):  # only train textcat
        ...
        init_tok2vec = './pretrained-model/starspace/' + 'model' + str(pretrain_epoch-1) + '.bin' 
        if init_tok2vec is not None:
            with Path(init_tok2vec).open('rb') as file_:
                textcat.model.tok2vec.from_bytes(file_.read())
        print("Training the model...")
        print("{:^5}\t{:^5}\t{:^5}\t{:^5}".format("LOSS", "P", "R", "F"))
        batch_size = compounding(16.0, 32.0, 1.001)
...
MAIN_ARGS = {
    "pretrain_epoch": ("select the pretrained weight trained by pretrain_epoch iterations", 'positional', None, int), 
